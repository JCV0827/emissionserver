...
// Helper to get a project's current stage for this user (project_members overrides base stage)
function getProjectCurrentStageForUser(projectId, userId) {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT 
        COALESCE(pm.current_stage, uh.stage) AS current_stage,
        COALESCE(pm.progress_status, uh.status) AS progress_status
      FROM user_history uh
      LEFT JOIN project_members pm ON pm.project_id = uh.id AND pm.user_id = ?
      WHERE uh.id = ?
      LIMIT 1`;
    queryDatabase(sql, [userId, projectId], (err, rows) => {
      if (err) return reject(err);
      if (!rows || rows.length === 0) return resolve({ current_stage: null, progress_status: null });
      resolve(rows[0]);
    });
  });
}
  ...
  queryDatabase(getAllProjectIdsQuery, [userId, userId], (err, projectIds) => {
    ...
    const projectsQuery = `
      SELECT 
        ...
        pm.role, 
        pm.progress_status,
        -- Code analysis aggregates
        (SELECT COUNT(*) FROM user_history ch WHERE ch.project_id = uh.id AND ch.entry_type = 'code' AND ch.is_deleted = 0) AS code_analyses_count,
        (SELECT COALESCE(SUM(ch.carbon_emit), 0) FROM user_history ch WHERE ch.project_id = uh.id AND ch.entry_type = 'code' AND ch.is_deleted = 0) AS code_emissions_sum_gco2,
        CASE 
    ...
    queryDatabase(projectsQuery, queryParams, (err, projects) => {
      ...
      // Optional filtering by query param
      const filter = (req.query && req.query.filter) || '';
      if (filter === 'codeOnly') {
        processedProjects = processedProjects.filter(p => (p.code_analyses_count || 0) > 0);
      }
...
app.post('/add_code_analysis', authenticateToken, async (req, res) => {
  ...
  if (!project_id || !stage || emissions_gco2 == null || energy_kwh == null) {
    return res.status(400).json({ error: 'Missing required fields' });
  }
  ...
  try {
    const accessAllowed = await new Promise((resolve, reject) => {
    ...
    const { current_stage } = await getProjectCurrentStageForUser(project_id, userId);
    if (!current_stage) {
      return res.status(400).json({ error: 'Unable to determine current project stage' });
    }
    const reqIdx = CANONICAL_STAGES.indexOf(stage);
    const curIdx = CANONICAL_STAGES.indexOf(current_stage);
    if (curIdx === -1) {
    ...
    queryDatabase(baseSql, [project_id], (baseErr, baseRows) => {
      ...
      if (err) {
        ...
        return res.status(500).json({ error: 'Database error (did you run the user_history migration to add entry_type, is_deleted, energy_kwh, eco_score, time_complexity, space_complexity, carbon_unit, source?)' });
      }
      // Return accumulated emissions for this stage to update UI
      ...
      queryDatabase(sumSql, [project_id, stage], (sumErr, sumRows) => {
        if (sumErr) {
          ...
          return res.status(201).json({ message: 'Code analysis added', id: result.insertId });
        }
...
app.delete('/code_analysis/:id', authenticateToken, (req, res) => {
  ...
  // Ensure it is a code entry and the user has access to the project
  const fetchSql = `
